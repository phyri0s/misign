...
- **Status**: accepted (data model implemented in PHY-98)
- **Date**: 2026-09-26
...
- `Signature`: list of strokes and their bounding box. `fitInto` gives the matrix that maps it into a box in displayed coordinates (the page as displayed, see `PageGeometry`), scaled uniformly, centred and turned the right way up, so one drawing fits any box; `displayedToUser()` then takes it to user space
